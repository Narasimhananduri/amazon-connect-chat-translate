...
if (newMessage === "") return;
        // Detect target language const destLang = languageTranslate.find(o => o.contactId === currentContactId[0]);console.log("Destination Language Object:", destLang);
// Translate agent message const translatedMessageAPI = await translateTextAPI(newMessage, 'en', destLang.lang);const translatedMessage = translatedMessageAPI.TranslatedText;console.log(`Original: ${newMessage} | Translated: ${translatedMessage}`);
// Update chat UI locally const newChat = {
  contactId: currentContactId[0],
  username: agentUsername,
  content: <p>{newMessage}</p>,
  translatedMessage: <p>{translatedMessage}</p>,
};addChat(prev => [...prev, newChat]);setNewMessage("");
// Retrieve the session for this contact const session = (() => {
...
// Send text message to Connect await sendMessage(session, translatedMessage);
// ✅ Handle voice contact — trigger Lambda try {
...
lang.contactId === currentContactId[0] ? lang.lang : null
 ...
 <ul className="chats" ref={messageEl}>
  ...
  <form className="input" onSubmit={handleSubmit}>
    <inputref={input}maxLength="1024"type="text"value={newMessage}onChange={e => setNewMessage(e.target.value)}
    />
    <input type="submit" value="Submit" />
  </form></div>
);
